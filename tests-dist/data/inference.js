...
function inferSimplicesLegacy(contexts, settings) {
    ...
    contexts.forEach(ctx => {
        ...
        if (!invertedIndex.has(folderKey))
            invertedIndex.set(folderKey, new Set());
        ...
        triadCandidates.forEach(a => {
            ...
            for (let i = 0; i < aPartners.length; i++) {
                for (let j = i + 1; j < aPartners.length; j++) {
                    ...
                    if (sortedNodes[0] !== a)
                        continue; // Only process when 'a' is the first in sorted order
    ...
    logger_1.logger.debug("inference", "Rebuilt inferred simplices (optimized)", {
