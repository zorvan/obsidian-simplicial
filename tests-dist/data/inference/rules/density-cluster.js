...
exports.detectDensityClusters = detectDensityClusters;
...
function averageInternalStrength(nodes, graph) {
    if (nodes.length < 2)
        return { average: 0, density: 0 };
    let total = 0;
    let connected = 0;
    const pairCount = (nodes.length * (nodes.length - 1)) / 2;
    for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
            const strength = (0, graph_1.getEdgeStrength)(nodes[i], nodes[j], graph);
            total += strength;
            if (strength > 0)
                connected++;
        }
...
function detectDensityClusters(graph, config) {
    ...
    for (const nodeId of graph.nodes.keys()) {
        if (visited.has(nodeId))
            continue;
        const seedNeighbors = (0, graph_1.getNeighborsAbove)(nodeId, denseThreshold, graph);
        if (seedNeighbors.length < minCoreNeighbors)
            continue;
        ...
        while (queue.length > 0) {
            ...
            const neighbors = (0, graph_1.getNeighborsAbove)(current, denseThreshold, graph);
            if (neighbors.length < minCoreNeighbors)
                continue;
        ...
        const trimmedCluster = sortedCluster.length > 5
            ? sortedCluster
                .map((id) => ({
                id,
                strength: (0, graph_1.getNeighborsAbove)(id, denseThreshold, graph)
                    .filter((neighbor) => cluster.has(neighbor))
                    .reduce((sum, neighbor) => sum + (0, graph_1.getEdgeStrength)(id, neighbor, graph), 0),
            }))
        ...
        if (stats.density < minDensity || stats.average < minAverageStrength)
            continue;
